...
def panelPolar(wing):
    ...
    n = 40;                                                                     # Degree of the Legendre polynomial approximation
    wcl = np.zeros([wing.getR(),n+1]);
    wcd = np.zeros([wing.getR(),n+1]);
    wcm = np.zeros([wing.getR(),n+1]);
    ...
    if np.any(wing.getPolD()):
        ...
        for jj in range(wing.getR()/2):
            ...
            [wcl[jj,:],wcd[jj,:],wcm[jj,:]] = intrapolLegend(Polars,n);
        ...
        [wcl[0,:],wcd[0,:],wcm[0,:]] = intrapolLegend(Polars,n);
    ...
    return wcl, wcd, wcm, al0, alphaMax, cl_alpha;
...
def vtailPolar(vtail):
    ...
    n = 40;                                                                     # Degree of the Legendre polynomial approximation
    wcl = np.zeros([vtail.getR(),n+1]);
    wcd = np.zeros([vtail.getR(),n+1]);
    wcm = np.zeros([vtail.getR(),n+1]);
...
def StalledPolar():
    ...
    return wcl, wcd, wcm;
def AeroCoef(wcl,wcd,wcm,alpha):
    """
    Autor : Quentin Borlon
    ...
    cd = np.zeros(n);
    cm = np.zeros(n);
    alphaD = 180./m.pi*alpha;
    for ii in range(n):
        cl[ii] = np.polyval(wcl[ii], alphaD[ii]);
        cd[ii] = np.polyval(wcd[ii], alphaD[ii]);
        cm[ii] = np.polyval(wcm[ii], alphaD[ii]);
    return cl,cd,cm;