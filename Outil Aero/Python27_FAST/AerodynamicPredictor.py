...
def VLM_Solve(invA,Vx,Vy,Vz,velTriX,velTriY,velTriZ,normal):
...
def AeroPredVTail(vtail,flow,lin,S,MAC,rp):
    ...
    y = vtail.getYP();
    z = vtail.getZP();
    [waPanel, wclPanel, wcdPanel, wcmPanel, alphal0PanelVLM, aMax, cl_alphaPanel] = p.vtailPolar(vtail);
    tol = 1e-4;                                                                 # Tolerance for the a.o.a. correction loop
    ...
    V0 = flow.getV0();
    [A,normal,Vx,Vy,Vz] = VLM.ICMatrixV(vtail,cl_alphaPanel);
    invA = np.linalg.inv(A);
#    [A,normal,Vx,Vy,Vz,select,select2] = VLM.ICMatrixV(vtail,cl_alphaPanel,flow);
    nbPan = len(y);
    nbA = len(Alphas);
    ...
    if lin:
        for ii in range(nbA):
            ...
            # Pour le VLM, comme la géométrie est prise en compte dans la matrice 
            # A_{i,j}, il faut donner le triangle des vitesses dans le repère absolu.
            velTriX = V0 * np.cos(AOA) * np.cos(-Beta) * np.ones(nbPan);
            velTriY = V0 * np.sin(-Beta);
            velTriZ = V0 * np.sin(AOA) * np.cos(-Beta) * np.ones(nbPan);
            AOA_VLM = np.arctan2(velTriY,velTriX);
            
            
            VxA = V0 * np.cos(Alphas[ii]) * np.cos(-Beta) * np.ones(nbPan);
            VyA = V0 * np.sin(-Beta) * np.ones(nbPan);
            VzA = V0 * np.sin(Alphas[ii]) * np.cos(-Beta) * np.ones(nbPan);
            velTriXLoc,velTriYLoc,velTriZLoc = u.localVelTriVT(VxA,VyA,VzA,sweepC4);
            
            a0 = np.arctan2(velTriYLoc,velTriXLoc);
            
            Norme = np.sqrt(velTriXLoc**2 + velTriYLoc**2);
            
            [Vix,Viy,Viz,ccl] = VLM_SolveV(invA,Vx,Vy,Vz,velTriX,velTriY,velTriZ,normal)
            clw = ccl/c;
            
            VxEnd = V0 * np.cos(Alphas[ii]) * np.cos(-Beta) * np.ones(nbPan) + Vix * m.cos(Alphas[ii]) - Viz * m.sin(Alphas[ii]);
            VyEnd = V0 * np.sin(-Beta) * np.ones(nbPan) + Viy;
            VzEnd = V0 * np.sin(Alphas[ii]) * np.cos(-Beta) * np.ones(nbPan) + Viz * m.cos(Alphas[ii]) + Vix * m.sin(Alphas[ii]);
            velTriXLocF,velTriYLocF,velTriZLocF = u.localVelTriVT(VxEnd,VyEnd,VzEnd,sweepC4);
            
            af = np.arctan2(velTriYLocF,velTriXLocF);
                    
            [clVisc,cd,cm] = p.AeroCoef(waPanel,wclPanel,wcdPanel,wcmPanel,af);
            
            speedFactor = (velTriXLocF*velTriXLocF+velTriYLocF*velTriYLocF)/(V0*V0);
            al_i[:,ii] = (a0 - af);
        ...
        return CY,CM,Cl,Cn,CDi,CD0,Alphas
    else:
        nCorrection = 0;
        K = np.ones(nbPan);
        for ii in range(nbA):                                              # if possible no numerical damping to fasten the computation
            ...
            # A_{i,j}, il faut donner le triangle des vitesses dans le repère absolu.
            velTriX = V0 * np.cos(AOA) * np.cos(-Beta) * np.ones(nbPan);
            velTriY = V0 * np.sin(-Beta) * np.ones(nbPan);
            velTriZ = V0 * np.sin(AOA) * np.cos(-Beta) * np.ones(nbPan);
            AOA_VLM = np.arctan2(velTriY,velTriX);
            AOA_VLM0 = AOA_VLM;
            ...
            [Vix,Viy,Viz,ccl] = VLM_SolveV(invA,Vx,Vy,Vz,velTriX,velTriY,velTriZ,normal)
            clw = -ccl/c;
            VxV = V0 * np.cos(Alphas[ii]) * np.cos(-Beta) * np.ones(nbPan) + Vix;
            VyV = V0 * np.sin(-Beta) * np.ones(nbPan) + Viy;
            VzV = V0 * np.sin(Alphas[ii]) * np.cos(-Beta) * np.ones(nbPan) + Viz;
            velTriXLoc,velTriYLoc,velTriZLoc = u.localVelTriVT(VxV,VyV,VzV,sweepC4);
            alphaLoc = np.arctan2(velTriYLoc,velTriXLoc);
            
            [clVisc,cd,cm] = p.AeroCoef(waPanel,wclPanel,wcdPanel,wcmPanel,alphaLoc);
            # apparent inviscid difference of a.o.a. such that the inviscid lift at AOA+deltaAlphas
            deltaAlpha = (clVisc-clw) / cl_alphaPanel*K; #viscous lift at alphaLoc
            nIter = 0;
            ...
            while np.amax(np.absolute(deltaAlpha))*2*m.pi > tol or np.any(np.isnan(deltaAlpha)):
                ...
                if nIter == nIterMAX or np.any(np.isnan(deltaAlpha)):
                    ...
                    if tol > tolStop:
                        ...
                        if nCorrection == 1:
                            ...
                            velTriX = V0 * np.cos(AOA) * np.cos(-Beta) * np.ones(nbPan);
                            velTriY = V0 * np.sin(-Beta - alphal0PanelVLM);
                            velTriZ = V0 * np.sin(AOA) * np.cos(-Beta) * np.ones(nbPan);
                ...
                velTriY = Norme * np.sin(AOA_VLM);
                
                [Vix,Viy,Viz,ccl] = VLM_Solve(invA,Vx,Vy,Vz,velTriX,velTriY,velTriZ,normal);
                clw = -ccl/c;
            ...
            speedFactor = (velTriXLoc*velTriXLoc+velTriYLoc*velTriYLoc)/(V0*V0);
            al_i[:,ii] =  (a0 - alphaLoc);
            if np.amax(np.absolute(deltaAlpha))*2.*m.pi >= tolStop or stop:
                ...
                print('Attention, the tolerance on cl for alpha = '+str(180./m.pi*Alphas[ii])+' has raised up to '+str(np.amax(np.absolute(deltaAlpha)*cl_alphaPanel)));
            nIterMAX = 25;
            tol = 1e-4;                                                         # reset the tolerance for next computation
...
def LooselyCoupled(ac,flow,lin):
    ...
    [A,normal,Vx,Vy,Vz] = VLM.ICMatrix(ac,cl_alphaPanel);
    invA = np.linalg.inv(A);
                ...
                if nIter == nIterMAX or np.any(np.isnan(deltaAlpha)):
                    ...
                    if max(K) < 0.5:
                        ...
                        if nCorrection == 1:
                            ...
                            velTriY = V0 * np.sin(-Beta) + viy;
                            velTriZ = V0 * np.sin(AOA) * np.cos(-Beta) + viz;
                            velTriX = Norme * np.cos(AOA - alphal0PanelVLM);
                            velTriZ = Norme * np.sin(AOA - alphal0PanelVLM);
                            VxA = V0 * np.cos(Alphas[ii]) * np.cos(-Beta) + vix;
                            VyA = V0 * np.sin(-Beta) + viy;
                            VzA = V0 * np.sin(Alphas[ii]) * np.cos(-Beta) + viz;
                            velTriXLoc,velTriYLoc,velTriZLoc = u.localVelTri(VxA,VyA,VzA,tp,sweepC4,dih);
                            ...
                            velTriY = V0 * np.sin(-Beta) + viy;
                            velTriZ = V0 * np.sin(AOA) * np.cos(-Beta) + viz;
                            Norme = np.sqrt(velTriX*velTriX+velTriZ*velTriZ);
                            velTriX = Norme * np.cos(AOA - alphal0PanelVLM);
                            velTriZ = Norme * np.sin(AOA - alphal0PanelVLM);
                            VxA = V0 * np.cos(Alphas[ii]) * np.cos(-Beta) + vix;
                            VyA = V0 * np.sin(-Beta) + viy;
...
def StronglyCoupled(ac,flow,lin):
    ...
    stop = False;
    # Construction of the A matrix
    [A,normal,Vx,Vy,Vz,select,select2] = VLM.ICMatrix(ac,cl_alphaPanel,flow);
    nA = np.shape(A)[0];
